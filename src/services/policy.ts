import { constants, SequencerProvider, RpcProvider, RPC, number, InvocationsDetailsWithNonce, Invocation, shortString, Signature, TransactionSimulationResponse, TransactionTraceResponse, FunctionInvocation } from "starknet";
...
const getTrace = async (transaction: Invocation & InvocationsDetailsWithNonce): Promise<TransactionTraceResponse> => {
  ...
  const trace: TransactionSimulationResponse = await provider.getSimulateTransaction(transaction, transaction);
  return trace.trace
}
