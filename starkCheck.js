  ...
  (trace.selector == approveSelector ||
    ...
    ? [trace].concat(
        trace.internal_calls.length
          ? trace.internal_calls.flatMap((it) => extractEvents(it))
          : []
      )
    : trace.internal_calls.length
    ? trace.internal_calls.flatMap((it) => extractEvents(it))
...
const res = events.filter((event) =>
  policyJSON.policy.reduce(
    (flag, policy) =>
      flag ||
      (event.caller_address == owner &&
...
const explainEvent = (event) =>
  console.log(
    'function on contract ' +
      ...
      'is trying to approve or send more than your policy'
  );

const explainFlag = (events) => events.map(explainEvent);
